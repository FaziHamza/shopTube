...
@Component({
  ...
  <nz-range-picker [ngClass]="to.config?.wrapper && to.config?.wrapper == 'floating_filled' || to.config?.wrapper == 'floating_outlined' ? to.config?.floatFieldClass : ''" nzMode="date" style="width:100%" [nzDisabled]="to.disabled" [formControl]="formControl" [nzStatus]="to.config?.status"></nz-range-picker>
  <label *ngIf="to.config?.wrapper == 'floating_filled' || to.config?.wrapper == 'floating_outlined'" [ngClass]="to.config?.floatLabelClass">{{to.label}}</label>
    ...
    <nz-range-picker class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" nzMode="date" style="width:100%" [nzDisabled]="to.disabled" [formControl]="formControl" [nzStatus]="to.config?.status"></nz-range-picker>
