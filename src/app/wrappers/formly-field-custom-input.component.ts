...
<div [ngClass]="to.config?.wrapper && to.config?.wrapper == 'floating_filled' || to.config?.wrapper == 'floating_outlined' || to.config?.wrapper == 'floating_standard' ? 'height' : ''">
   <ng-container *ngIf="to.config?.wrapper != 'floating_standard'">
      <nz-input-group  *ngIf="!to.config?.suffixicon && !to.config?.prefixicon" [nzSuffix]="to.config?.addonLeft" [nzPrefix]="to.config?.addonRight" [nzStatus]="to.config?.status" [nzSize]="to.config?.size">
      ...
      <nz-input-group  *ngIf="to.config?.suffixicon || to.config?.prefixicon" [nzSuffix]="suffixTemplateInfo" [nzPrefix]="prefixTemplateUser" [nzSize]="to.config?.size" [nzStatus]="to.config?.status" [nzSize]="to.config?.size">
      <input [ngClass]="to.config?.wrapper && to.config?.wrapper == 'floating_filled' || to.config?.wrapper == 'floating_outlined' ? to.config?.floatFieldClass : ''" nz-input *ngIf="!to['maskString']" class='form-control' [formControl]="formControl" [type]="type" [formlyAttributes]="field" [nzStatus]="to.config?.status">
      <input [ngClass]="to.config?.wrapper && to.config?.wrapper == 'floating_filled' || to.config?.wrapper == 'floating_outlined' ? to.config?.floatFieldClass : ''" nz-input *ngIf="to['maskString']" [formControl]="formControl" [mask]="to['maskString']" class='form-control' [type]="type" [formlyAttributes]="field" aria-describedby="passwordHelpBlock" [nzStatus]="to.config?.status">
      <label *ngIf="to.config?.wrapper == 'floating_filled' || to.config?.wrapper == 'floating_outlined'" [ngClass]="to.config?.floatLabelClass">{{to.label}}</label>
      </nz-input-group>
      <ng-template #prefixTemplateUser>
         <span>
         <st-icon *ngIf="to.config?.suffixicon" [type]="to.config?.iconType || 'outline'" [icon]="to.config?.suffixicon"
         [size]="to.config?.iconSize" [color]="to.config?.iconColor"></st-icon>
         </span>
      </ng-template>
      <ng-template #suffixTemplateInfo>
         <span>
         <st-icon *ngIf="to.config?.prefixicon" [type]="to.config?.iconType || 'outline'" [icon]="to.config?.prefixicon"
         [size]="to.config?.iconSize" [color]="to.config?.iconColor" ></st-icon>
         </span>
      </ng-template>
   </ng-container>
   <div class="relative z-0" *ngIf="to.config?.wrapper == 'floating_standard'">
      <input nz-input class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" *ngIf="!to['maskString']"  [formControl]="formControl" [type]="type" [formlyAttributes]="field" [nzStatus]="to.config?.status">
      <input nz-input class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" *ngIf="to['maskString']" [formControl]="formControl" [mask]="to['maskString']"  [type]="type" [formlyAttributes]="field" aria-describedby="passwordHelpBlock" [nzStatus]="to.config?.status">
